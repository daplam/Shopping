...
Call log:
  ...
  - waiting for getByRole('row').filter({ has: getByText('Mango') }).locator('#cell-4-undefined')
    ...
    at C:\ShoppingBDD\tests\Excel\exceltest.spec.ts:73:59
```
...
```ts
  ...
  53 |     const [downloadPromise] = await Promise.all([
  54 |         page.waitForEvent('download'),
  55 |         await page.getByRole('button', { name: 'Download' }).click()
  56 |     ]);
  57 |     // Guarda el archivo en una ruta personalizada
  58 |     const path = await downloadPromise.path(); // esto es un path temporal
  59 |
  60 |     // Para guardarlo en un lugar específico:
  61 |     await downloadPromise.saveAs('/Users/Daplam/Downloads/download.xlsx');
  62 |
  63 |     expect(await downloadPromise.suggestedFilename()).toContain('download.xlsx');
  64 |
  65 |
  66 |     await page.pause()
  67 |     writeExcel({ searchValue: textSearch, replaceValue: updateValue, excelFile: "/Users/Daplam/Downloads/download.xlsx" });
  68 |
  69 |     await page.locator("#fileinput").click();
  70 |     await page.locator("#fileinput").setInputFiles("/Users/Daplam/Downloads/download.xlsx");
  71 |     const textlocator = page.getByText(textSearch);
  72 |     const desiredRow = await page.getByRole('row').filter({ has: textlocator });
> 73 |     await expect(desiredRow.locator("#cell-4-undefined")).toContainText(updateValue);
     |                                                           ^ Error: Timed out 5000ms waiting for expect(locator).toContainText(expected)
  74 | })
```