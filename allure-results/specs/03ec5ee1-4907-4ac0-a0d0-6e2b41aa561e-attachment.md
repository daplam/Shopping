...
Call log:
  ...
  - waiting for getByRole('row')
    ...
    at C:\ShoppingBDD\tests\Excel\exceltest.spec.ts:117:41
```
...
```ts
  ...
  113 |
  114 |
  115 |     const textlocator = page.getByText(textSearch);
  116 |     const desiredRow = await page.getByRole('row').filter({ has: textlocator })
> 117 |     await expect(page.getByRole('row')).toContainText(textSearch)
      |                                         ^ Error: expect.toContainText: Error: strict mode violation: getByRole('row') resolved to 7 elements:
