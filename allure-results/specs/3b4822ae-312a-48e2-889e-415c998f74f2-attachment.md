...
Call log:
  ...
  - waiting for getByRole('row').filter({ has: getByText('Mango') }).locator('#cell-4-undefined')
    ...
    at C:\ShoppingBDD\tests\Excel\exceltest.spec.ts:78:59
```
...
```ts
  ...
  60 |     ]);
  61 |     // Guarda el archivo en una ruta personalizada
  62 |     const path = await promiseDownload.path(); // esto es un path temporal
  63 |
  64 |     // Para guardarlo en un lugar específico:
  65 |     await promiseDownload.saveAs('/Users/Daplam/Downloads/download.xlsx');
  66 |
  67 |     // Puedes hacer más validaciones si quieres
  68 |     expect(await promiseDownload.suggestedFilename()).toContain('download.xlsx');
  69 |
  70 |     //await promiseDownload
  71 |     await page.pause()
  72 |     writeExcel({ searchValue: textSearch, replaceValue: updateValue, excelFile: "/Users/Daplam/Downloads/download.xlsx" });
  73 |
  74 |     await page.locator("#fileinput").click();
  75 |     await page.locator("#fileinput").setInputFiles("/Users/Daplam/Downloads/download.xlsx");
  76 |     const textlocator = page.getByText(textSearch);
  77 |     const desiredRow = await page.getByRole('row').filter({ has: textlocator });
> 78 |     await expect(desiredRow.locator("#cell-4-undefined")).toContainText(updateValue);
     |                                                           ^ Error: Timed out 5000ms waiting for expect(locator).toContainText(expected)
  79 | })
```